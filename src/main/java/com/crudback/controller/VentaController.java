...
public class VentaController {
	...
	@PutMapping()
	public ResponseEntity<Venta> modificar(@RequestBody Venta p) throws Exception{
		Venta obj = service.modificar(p);
		return new ResponseEntity<Venta>(obj,HttpStatus.OK);
	}
	
	@DeleteMapping("/{id}")
	public ResponseEntity<Venta> eliminar(@PathVariable("id") Integer id) throws Exception{
		Venta obj=service.listarPorId(id);	
		if(obj==null) {
			throw new ModeloNotFoundException("ID NO ENCONTRADO " + id);
		}
		...
		return new ResponseEntity<Venta>(obj,HttpStatus.NO_CONTENT);
	}
